...
VideoStable::VideoStable(const string inputPath, const string outputPath, const string feature_type) {
...
void VideoStable::run() const {
    ...
    while(true) {
        ...
        Mat T = calcRigidTransform(prev_grey, cur_grey);
        warpAffine(cur, cur, T, cur.size());
        cur.copyTo(prev);
...
Mat VideoStable::calcRigidTransform(const Mat &img1, const Mat &img2) const {
